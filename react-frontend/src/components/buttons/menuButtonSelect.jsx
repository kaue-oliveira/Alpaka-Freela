...
const DynamicButton = ({ label, onClick, isSelected }) => {
  ...
  const buttonStyles = {
    container: {
      ...
      fontSize: "14px",
      cursor: "pointer",
      backgroundColor: isSelected ? "#e7e5fd" : isHovered ? "#e7e5fd" : "#f5f5f5",
...
const MenuButtonSelect = ({ buttons, onButtonClick }) => {
  const defaultSelectedIndex = buttons.findIndex(button => button.startSelected);
  const [selectedButton, setSelectedButton] = useState(defaultSelectedIndex !== -1 ? defaultSelectedIndex : null);
