...
public class SpringBootBackEndApplication implements CommandLineRunner{
       ...
       if (usuariosAtuais.size() < 50) {
            ...
            for (int i = 0; i < 50; i++) {
                try {
                    ...
                    byte[] profileImageBytes = downloadImageAsBlob(imageUrl);
                    ...
                    System.out.println("new user -> " + usuario.getNome());
                } catch (Exception e) {
            ...
            for (int i = 0; i < usuarios.size(); i++) {
                ...
                OfertaDeServico ofertaDeServico = new OfertaDeServico(
                        new DadosCadastroOfertaDeServico(descricao, valorCobrado, tecnologiasIds, habilidadesIds), usuarios.get(i)
                );
            ...
            for (int i = 0; i < usuarios.size(); i++) {
                ...
                OfertaDeTrabalho ofertaDeTrabalho = new OfertaDeTrabalho(
                        new DadosCadastroOfertaDeTrabalho(titulo, descricao, pagamento, tecnologiasIds), usuarios.get(i)
                );
    ...
    private byte[] downloadImageAsBlob(String imageUrl) throws Exception {
